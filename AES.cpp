...
void shiftRowsLeft(unsigned char* state) {
...
void reorderColumns(unsigned char* state) {
    unsigned char tmp[16];
...
void shiftRowsRight(unsigned char* state) {
    //each row is shifted 1 byte to the right
    unsigned char tmp[16];

    for (int i = 0; i < 16; i++) {
        int stateLoc = (16* (1 + i / 5) - i * 3) % 16;
        tmp[i] = state[stateLoc];
    }
...
void initialRound(unsigned char* state, const unsigned char* key) {
    //on the first round we don't need to mix columns
    //like the finalRound but with the steps executed backwards
    addRoundKey(state, key); //the reverse of XOR-int each element is XOR-ing it again
    shiftRowsRight(state);
    substituteBytes(state, inverse_s_box);

}

void inverseRound(unsigned char* state, unsigned char* key) {
    //each operation is in the opposite order and does the opposite to the ones in the forward round
    addRoundKey(state, key);
...
void decrypt(unsigned char* encryptedMessage, unsigned char* expandedKey, unsigned char* decryptedMessage) {
    //the decryption follows the same process as the encryption, only in the reverse order
